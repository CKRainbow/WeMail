...
namespace WeMail.Common.Helpers
{
    public static class MiniDump
    {
        ...
        public static void TryDump(string path, string process_name)
        {
            ...
            foreach (Process process in process_list) //create dump for each process
            {
                ...
                string out_dump_path = Path.Combine(
                    path,
                    $"{process_name}_{ProcessId.ToString()}_{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}.dmp"
                );
                FileStream procdumpFileStream = File.Create(out_dump_path);
                bool _ = MiniDumpWriteDump(
                    hProcess,
                    ProcessId,
                    procdumpFileStream.SafeFileHandle,
                    DumpType,
                    IntPtr.Zero,
                    IntPtr.Zero,
                    IntPtr.Zero
